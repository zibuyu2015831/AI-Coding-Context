...
- 即使密码、Token、固定 key 来自开源仓库配置，也不应在 AICC 正式文档中复述具体值。
- 允许写变量名、配置文件路径、风险说明和生产环境改写要求。
- 不要把默认用户名、数据库名、密码提示组合成可直接复制的连接凭据。
- 如果用户明确要求可复制部署脚本，密码和 key 位置使用占位变量，例如 `<DB_PASSWORD>`、`<APP_KEY>`。
