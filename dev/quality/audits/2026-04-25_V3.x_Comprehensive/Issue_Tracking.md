...
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：作为统领条目，由 011/012/014/015 联合修复闭合；FRAMEWORK_CONTEXT 顶部摘要简化为概览快照 + SSOT 指针，PROGRESS 作为单一详细真相源；详见 `Review_Verification_Report.md` Batch 2#001）
...
- **状态**: 🟢 已修复（B5#008 — 2026-04-26；C4 commit 删除 L274-L293 旧重复段 + 孤字"发布" + 未闭合 fence；同 commit 附带清理 B2 附带 L87 015 误列 + 007 衍生 HOW_TO L281 待创建标记）
...
- **归属视角**: C（dev 卫生）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B5#009 — 2026-04-26；C5 commit L699 改路径为 confirmed/018 + 加注历史分析）
...
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：顶部 L11-L23 改写为 P0/P1/P2/V3.0+ 分组的进度概览 + SSOT 指针；底部 L703 (10/17)→(13/19)，列出全 13 项；frontmatter 新增 `progress_synced_at` 字段；详见 `Review_Verification_Report.md` Batch 2#011）
...
- **归属视角**: B（完整性）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：PROGRESS L18 已完成 11→13 项，含 011 + 019；L34 P1 计数 011 🟢→✅ + 全 5 项 ✅；L15 总数 18→19；L16 confirmed 13→14；详见 `Review_Verification_Report.md` Batch 2#012）
...
- **归属视角**: B+C
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：FRAMEWORK_CONTEXT L539-L540 路径前缀 reference/→core/，与 L744-L745 一致；详见 `Review_Verification_Report.md` Batch 2#013）
...
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：原"剩余关键能力（规划中）"段彻底重写为"V3.0 进度概览"（P0/P1/P2/V3.0+ 分组），删除将已完成项列为规划中的矛盾措辞；纠正 015 实为已归档非"剩余"；详见 `Review_Verification_Report.md` Batch 2#014）
...
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：⚠️ **原方案有重大缺陷** — 称"019 实体不存在需创建"，但 confirmed/019-systematic-review-framework.md 单文件实体已存在并符合 022 修复后命名规范；已重写：仅做 PROGRESS 登记不重组实体。PROGRESS 末尾追加"V3.0+ 后期增益"小节登记 019；总数 18→19；FRAMEWORK_CONTEXT 联动；详见 `Review_Verification_Report.md` Batch 2#015）
...
- **归属视角**: B（完整性）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26："两层修复"短期：改为 `tools/py/tests/` 实际路径并明示 doc_error/doc_fix 测试尚未补全；长期补全任务标注待入 PROGRESS；详见 `Review_Verification_Report.md` Batch 2#018）
...
- **状态**: 🟢 已修复（B3 修复 — 实施 fallback 选项 B + 修复内部 load_config("") bug，2026-04-26）
...
- **状态**: 🟢 已修复（B2 复审 + 修复 2026-04-26：补 _templates/ 索引 2 项；examples 21→18 加 design_thinking/ 子目录注；language_specific 7→6 含 base/ 子目录；personas 加注"3 个角色 + 1 README"口径；详见 `Review_Verification_Report.md` Batch 2#023）
...
- **归属视角**: B（完整性）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B3 修复 — 删除两项 + 重组为"必需 5 件套 + 可选附加"二级结构，2026-04-26）
...
- **状态**: 🟢 已修复（B3 修复 — 重写 L44-L58 为"按需生成"语境，2026-04-26）
...
- **状态**: 🟢 已修复（B3 修复 — 创建 workflows/complexity_alert_workflow.md 含 frontmatter + 补 AI_ENTRY_POINT 索引 + path_d @complexity 章节，同步关闭 019，2026-04-26）
...
- **归属视角**: A（用户）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B5#028 — 2026-04-26；C1 合并 commit 含 028+030[quick_start 部分]+033）
...
- **归属视角**: A（用户）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B5#030 — 2026-04-26；实际 Public 层共 13 处而非 Issue 列出的 4 处，C1+C3 双 commit 全清；治理规则待入 SSOT）
...
- **归属视角**: A（用户）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B5#031 — 2026-04-26；C2 commit 标题改 4 步）
...
- **状态**: 🟢 已修复（B5#033 — 2026-04-26；C1 合并 commit 改 V3.0 工具优先 + shell fallback）
...
- **状态**: 🟡 修复中（B3 修复 P0 完成 — 顶层入口 3/3 + core/ 21/21 = 100% 合规；strict 通过；Public 14%→22%；P1（workflows/guides）+ P2（agents/templates/config）待后续推进，2026-04-26）
...
- **归属视角**: C（dev 卫生）
- **关联任务/ADR**: 无
- **状态**: 🟢 已修复（B3 修复 — 添加完整 JSDoc 头部，对齐 .py 内容质量；JS 主脚本 docstring 合规率 100%，2026-04-26）
