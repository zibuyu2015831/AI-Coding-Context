...
│                        AICC 框架核心                             │
├─────────────────────────────────────────────────────────────────┤
│  AI_ENTRY_POINT.md → 文档生成器 → AI_Coding_Context.md         │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│                                                                  │
│  ┌─────────────────┐    ┌─────────────────┐                    │
│  │ KnowledgeLoader │ ← │ KnowledgeConfig │                    │
│  │   (知识加载器)   │    │   (配置管理)    │                    │
│  └────────┬────────┘    └─────────────────┘                    │
│           │                                                      │
│           ▼                                                      │
│  ┌─────────────────┐    ┌─────────────────┐                    │
│  │ KnowledgeMatcher│ ← │ GitRepository   │                    │
│  │   (知识匹配器)   │    │   (Git 仓库)    │                    │
│  └────────┬────────┘    └─────────────────┘                    │
│           │                                                      │
│           ▼                                                      │
│  ┌─────────────────┐    ┌─────────────────┐                    │
│  │  本地 knowledge │    │ 共享知识库缓存  │                    │
│  │   (项目级)      │    │   (共享级)      │                    │
│  └─────────────────┘    └─────────────────┘                    │
└─────────────────────────────────────────────────────────────────┘
```
...
```mermaid
graph LR
    A[AI 发起文档生成] --> B[KnowledgeLoader 加载配置]
    B --> C{Git 仓库已配置?}
    C -->|是| D[加载共享知识索引]
    C -->|否| E[跳过共享知识]
    D --> F[KnowledgeMatcher 分析项目特征]
    F --> G[匹配本地知识]
    G --> H[匹配共享知识]
    H --> I[合并知识结果]
    I --> J[生成文档时注入匹配的知识]
...
| AICC 工具 | 交互方式 | 说明 |
|-----------|----------|------|
| **project_scanner.js** | 扫描项目特征 | 提供技术栈、项目类型等信息供知识匹配 |
| **summary_extractor** | 提取知识摘要 | 从现有 knowledge 目录提取元数据 |
| **文档生成器** | 注入匹配知识 | 将匹配结果注入生成的文档 |
| **ADR 系统** | 共享决策库 | 提供架构决策模板和问题解决方案 |
| **复杂度仪表盘** | 引用最佳实践 | 从知识库引用性能优化和架构模式 |

### 4. 配置集成

知识复用功能通过 `.aicc/config.yml` 配置：
...
```markdown
<!-- 引用共享知识 -->
:::knowledge-ref pattern:rest-api-best-practices
:::

<!-- 指定语言示例 -->
:::knowledge-ref pattern:rest-api-best-practices language:python
:::

<!-- 带上下文的引用 -->
:::knowledge-ref problem:api-timeout-solution context:microservice
:::
```
...
| 指标 | 验收方式 | 说明 |
|------|----------|------|
| 知识引用准确率 > 90% | 测试集评估 | 使用标准测试集，包含 100 个知识查询场景，准确率 = 正确引用数 / 总查询数 |
| 知识匹配召回率 > 80% | 测试集评估 | 测试集包含 50 个项目特征场景，召回率 = 召回的相关知识数 / 知识库中实际相关知识数 |
| 版本兼容性检查准确率 > 85% | 测试场景评估 | 测试 20 个版本兼容性场景，准确识别版本冲突 |
| 知识库拉取成功率 > 99% | 网络模拟测试 | 在不同网络条件下测试 100 次拉取，成功率 = 成功数 / 总次数 |
| 配置解析正确率 > 98% | 配置测试 | 测试 50 种不同配置的解析，正确率 = 正确解析数 / 总配置数 |
| CLI 命令执行成功率 > 95% | 集成测试 | 测试所有 CLI 命令的正常和异常场景 |

**说明**：
- "用户满意度评分 > 4.0" 作为长期指标，在框架正式发布后通过用户反馈收集
- 当前阶段通过上述可量化指标确保质量