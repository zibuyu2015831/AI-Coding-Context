...
- [ ] 是否涉及跨越架构阈值的核心决策？（如引入重大依赖、更换基础协议、安全架构重构等）
- [ ] 结论如果是 "Yes"，则必须在此方案实施前，基于 `dev_docs/architecture/adr-template.md` 优先起草 ADR (Architecture Decision Record, 架构决策记录) 决策文档并附送人工审批。
...
### 步骤 2.6: 知识库匹配与引用解析 ⭐ (V3.0) `[MUST IF 存在知识库]`

**目的**: 自动从本地或共享知识库中检索并注入相关的最佳实践、架构模式和问题解决方案。

**AI 指令模板**:

```
检测到项目已配置知识库，请执行以下操作：
1. 扫描已生成的文档（主文档和子文档）中的知识点
2. 使用 `tools/py/knowledge_matcher.py` 解析文档
3. 自动注入匹配的共享知识内容
```
...
**质量检查**:
- [ ] 检查引用是否成功解析（无 `UNRESOLVED` 标记）
- [ ] 注入的内容与文档上下文契合
- [ ] 保持文档格式整洁

---

### 步骤 2.7: 为所有文档添加摘要 ⭐ (V3.0) `[MUST]`
