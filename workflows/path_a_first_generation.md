---
title: 路径 A 首次生成流程
summary: 定义目标项目尚未存在 dev_docs 文档体系时的完整首轮生命周期；默认单次执行只到 Step 7.5 Phase 1 人工审核门，用户确认后才进入正式生成。
...
> **默认执行边界**: 首次运行默认只生成 `_analysis` 三件套并停在 Step 7.5。除非用户明确授权“跳过审核/一次性生成正式文档”，否则不得在同一轮直接生成正式文档。
...
| 检测到的规模 (文件数) | 自动选择策略      | 用户确认后正式生成阶段的执行方式 |
| --------------------- | ----------------- | -------------------------------- |
...
> 上表只决定 Step 8 的正式文档生成方式，不改变 Step 7.5 的人工审核门。小型项目可以在用户确认后一次性完成正式文档，但仍必须先完成 Phase 1 方案复查并等待确认。
...
如果用户没有明确回复“方案审核通过”或等价确认，本流程到此结束。本阶段不得生成 `AI_Coding_Context.md`、子文档、`rules/combined/AI_RULES.md` 或 `health_check_report.md`。
...
> ⚠️ **进入条件**: `generation_progress.md` 必须记录用户确认或明确授权跳过审核的来源与文本摘要。
